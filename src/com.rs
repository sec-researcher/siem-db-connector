...
pub async fn send_data(
    ...
    match time::timeout(Duration::from_secs(2), TcpStream::connect(&partner_address))
        .await
        .unwrap_or(Err(Error::last_os_error()))
    {
        //match TcpStream::connect(partner_address).await {
        Ok(mut socket) => {
            match socket
                .write_all(&format!("{}{}{}", start_sign, change_track, end_sign).as_bytes())
                .await
            {
                Ok(_) => Ok(true),
                Err(e) => Err(e),
...
pub async fn send_data_get_response(
    partner_address: &str,
    data: &str,
    start_sign: &str,
    end_sign: &str,
) -> Result<String, ComError> {
    match TcpStream::connect(partner_address).await {
        Ok(mut socket) => {
            match socket
                .write_all(&format!("{}{}{}", start_sign, data, end_sign).as_bytes())
                .await
            {
                Ok(_) => match receive_data(&mut socket, "", "***END***", "".to_string()).await {
                    Ok(data) => Ok(data),
                    Err(e) => Err(e),
                },
                Err(e) => Err(ComError::IOError(e)),
            }
        }
        Err(e) => Err(ComError::IOError(e)),
    }
...
pub async fn receive_data(
    ...
    while msg.len() < end_sign_len || &msg[msg.len() - end_sign_len..] != end_sign {
        match time::timeout(Duration::from_secs(2), socket.read(&mut buf))
            .await
            .unwrap_or(Ok(0))
        {
            Ok(n) => {
                                        ...
                                        *state.lock() = State::Master;
                                        ...
                                        if message.len() == 2 {
                                            if message[1] != config_hash {
                                                match socket
                                                    .write_all(&format!("New config").as_bytes())
                                                    .await
                                                {
                                                    Ok(_n) => {
                                                    ...
                                                    Err(e) => log::error!(
                                                        "Error in requesting new config, OE: {}",
                                                        e
                                                    ),
                                                }
                                            }
        ...
        loop {
            ...
            match socket.read(&mut buf).await {
                ...
                Ok(n) => {
                    ...
                    let message = String::from_utf8(buf[..n].to_vec()).log_or(
                        "Can not convert process_incoming data to string",
                        "".to_string(),
                    );
                    if message == "What's up?\n" {
                        ...
                        if *state.lock() != State::Slave {
                                ...
                                .await
                            {
                                ...
                                Err(e) => log::error!(
                                    "Error in sending 'I'm master' message to client, OE: {}",
                                    e
                                ),
                            }
                        } else {
                            match socket.write_all(&format!("I'm slave\n").as_bytes()).await {
                                Ok(_) => {
                                    log::info!("'I'm slave' message sent to client successfully")
                                }
                                Err(e) => log::error!(
                                    "Error in sending 'I'm slave' message to client, OE: {}",
                                    e
                                ),
                            }
                        }
                        ...
                        match socket
                            .write_all(format!("{}***END***", log_sources_text).as_bytes())
                            .await
                        {
                            Ok(_) => log::info!("New config sent to client successfully"),
                            Err(e) => {
                                log::error!("Error in sending new config to client, OE: {}", e)
                            }
                        }
                        ...
                        match socket
                            .write_all(&format!("{}***END***", data).as_bytes())
                            .await
                        {
                            Ok(_) => log::info!("db_track_change for init sent successfully"),
                            Err(e) => {
                                log::error!("Error on sending init db_track_change, OE:{}", e)
                            }
                        }
...
pub async fn create_udp_sockets_concurrently(
    log_servers: &Vec<&str>,
    ...
    for connection in connections {
        ...
        match connection {
            ...
            Err(e) => log::error!(
                "Error in connecting to log server: {}, OE: {}",
                log_servers[i],
                e
            ),
        }
